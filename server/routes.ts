...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/sitemap.xml", async (req, res) => {
    try {
      ...
      const staticRoutes = [
        "", "/donate", "/campaigns", "/gallery", "/videos", "/blog", "/contact", "/policies", "/sitemap", "/login", "/register"
      ];
      ...
      campaigns.forEach(c => {
        xml += `  <url>\n    <loc>${baseUrl}/campaigns/${c.id}</loc>\n    <changefreq>weekly</changefreq>\n    <priority>0.7</priority>\n  </url>\n`;
      });
      ...
      policies.forEach(p => {
        xml += `  <url>\n    <loc>${baseUrl}/policies/${p.slug}</loc>\n    <changefreq>monthly</changefreq>\n    <priority>0.5</priority>\n  </url>\n`;
      });
  ...
  [uploadsDir, bannersDir, cardsDir, qrCodesDir, galleryDir, videosDir, blogDir, socialIconsDir, proofsDir, timelineDir].forEach(dir => {
  ...
  app.post("/api/upload/proof", proofUpload.single('file'), (req, res) => {
    try {
      if (!req.file) {
      ...
      res.status(500).json({ message: "Error uploading proof file", error: error instanceof Error ? error.message : String(error) });
    }
  });
  ...
  // Mount new feature routes (Timeline, Volunteers, Transparency, FAQs, Recurring Giving)
  ...
  app.post("/api/admin/stats", isAdmin, async (req, res) => {
    try {
      // Ensure suffix is always a string (DB column is NOT NULL)
      const body = { ...req.body, suffix: req.body.suffix ?? "" };
      const data = insertStatSchema.parse(body);
      const stat = await storage.createStat(data);
  ...
  app.put("/api/admin/stats/:id", isAdmin, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      // Ensure suffix is always a string (DB column is NOT NULL)
      const body = { ...req.body, suffix: req.body.suffix ?? "" };
      const data = insertStatSchema.partial().parse(body);
      const stat = await storage.updateStat(id, data);
      res.json(stat);
    } catch (error: any) {
  ...
  app.delete("/api/admin/stats/:id", isAdmin, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
  ...
  app.get("/api/user/donations", isAuthenticated, async (req, res) => {
    try {
      ...
      const enrichedDonations = await Promise.all(donations.map(async (d) => {
        let targetName = "";
        if (d.campaignId) {
          const campaign = await storage.getCampaign(d.campaignId);
          targetName = campaign?.title || "Unknown Campaign";
        } else if (d.categoryId) {
          const category = await storage.getDonationCategory(d.categoryId);
          targetName = category?.name || "Unknown Category";
        } else if (d.eventId) {
          const event = await storage.getEvent(d.eventId);
  ...
  app.get("/api/campaigns", async (req, res) => {
    try {
      ...
      const campaignList = allCampaigns.map(c => {
        const approvedDonations = allDonations.filter(d => d.campaignId === c.id && d.status === 'approved');
        const totalRaised = approvedDonations.reduce((sum, d) => sum + Number(d.amount), 0);
      ...
      if (isAdminUser || req.query.all === 'true') {
        ...
        res.json(campaignList.filter(c => c.status === 'active'));
      }
    } catch (error) {
  ...
  app.get("/api/campaigns/:id", async (req, res) => {
    try {
      ...
      const allDonations = await storage.getDonations();
      const approvedDonations = allDonations.filter(d => d.campaignId === id && d.status === 'approved');
      const totalRaised = approvedDonations.reduce((sum, d) => sum + Number(d.amount), 0);
      const donorCount = approvedDonations.length;
      
      const cards = await storage.getCampaignDonationCards(id);
      ...
      res.json({
        ...
        donorCount,
        cards,
        donations: approvedDonations.map(d => ({
          name: d.name,
          amount: d.amount,
      ...
      res.status(500).json({ message: "Error fetching campaign details", error: String(error) });
    }
  });

  app.post("/api/admin/campaigns", isAdmin, async (req, res) => {
    try {
      // Coerce date strings → Date objects so Zod timestamp fields accept them
      const body = { ...req.body };
      ...
      if (body.goalAmount && typeof body.goalAmount === 'string') body.goalAmount = parseInt(body.goalAmount, 10);
  ...
  app.patch("/api/admin/campaigns/:id", isAdmin, async (req, res) => {
    try {
      ...
      const data = insertCampaignSchema.partial().parse(body);
  ...
  // Preset donation cards endpoints
  app.get("/api/campaigns/:id/cards", async (req, res) => {
    try {
      const campaignId = parseInt(req.params.id);
      const cards = await storage.getCampaignDonationCards(campaignId);
      res.json(cards);
    } catch (error) {
      res.status(500).json({ message: "Error fetching donation cards" });
    }
  ...
  app.post("/api/admin/campaigns/:id/cards", isAdmin, async (req, res) => {
    try {
      const campaignId = parseInt(req.params.id);
      const data = insertDonationAmountCardSchema.parse({
        ...req.body,
        campaignId
      });
      const card = await storage.createCampaignDonationCard(data);
      res.json(card);
    } catch (error) {
      if (error instanceof z.ZodError) {
        return res.status(400).json({ message: "Invalid data", errors: error.errors });
      }
      res.status(500).json({ message: "Error creating donation card", error: String(error) });
    }
  });

  app.delete("/api/admin/campaigns/cards/:cardId", isAdmin, async (req, res) => {
    try {
      const cardId = parseInt(req.params.cardId);
      const success = await storage.deleteCampaignDonationCard(cardId);
      if (!success) {
        return res.status(404).json({ message: "Card not found" });
      }
      res.json({ message: "Donation card deleted successfully" });
    } catch (error) {
      res.status(500).json({ message: "Error deleting donation card" });
    }
  });

  // Campaign donation submission endpoint
  app.post("/api/campaigns/:id/donate", async (req, res) => {
    try {
      const campaignId = parseInt(req.params.id);
      const { name, email, phone, address, amount, amountType, paymentProofFile, message, panCard } = req.body;
      
      if (!name || !email || !phone || !amount || !paymentProofFile) {
        return res.status(400).json({ message: "Name, email, phone, amount, and payment proof are required" });
      }
      ...
      res.status(500).json({ message: "Error submitting donation", error: String(error) });
    }
  });

  // Review endpoint
  app.patch("/api/admin/donations/:id/review", isAdmin, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      ...
      const updated = await storage.updateDonation(id, {
        status,
      ...
      res.json({
        message: `Donation reviewed and ${status} successfully`,
        donation: updated
      });
    } catch (error) {
      res.status(500).json({ message: "Error reviewing donation", error: String(error) });
    }
  });

  // Receipt settings endpoints
  app.get("/api/admin/receipt-settings", isAdmin, async (req, res) => {
    try {
      const settings = await storage.getReceiptSettings();
      res.json(settings);
    } catch (error) {
      res.status(500).json({ message: "Error fetching receipt settings", error: String(error) });
    }
  });

  app.put("/api/admin/receipt-settings", isAdmin, async (req, res) => {
    try {
      const { orgName, orgAddress, orgEmail } = req.body;
      if (!orgName || !orgAddress || !orgEmail) {
        return res.status(400).json({ message: "Organization name, address, and email are required" });
      }
      const updated = await storage.updateReceiptSettings({ orgName, orgAddress, orgEmail });
      res.json({
        message: "Receipt settings updated successfully",
        settings: updated
      });
    } catch (error) {
      res.status(500).json({ message: "Error updating receipt settings", error: String(error) });
    }
  });
